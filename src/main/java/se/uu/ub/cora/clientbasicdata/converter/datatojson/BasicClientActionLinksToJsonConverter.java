...
public class BasicClientActionLinksToJsonConverter implements ClientDataToJsonConverter {
	...
	public BasicClientActionLinksToJsonConverter(JsonBuilderFactory jsonBuilderFactory,
			Map<ClientAction, ClientActionLink> actionLinks,
			ClientDataToJsonConverterFactory dataToJsonConverterFactory) {
	...
	private void addActionLinksToBuilderObject() {
		...
		for (Entry<ClientAction, ClientActionLink> actionLinkEntry : actionLinks.entrySet()) {
			ClientActionLink actionLink = actionLinkEntry.getValue();
	...
	private void possiblyAddBodyToBuilder(ClientActionLink actionLink,
			JsonObjectBuilder internalLinkBuilder) {
		if (actionLinkHasBody(actionLink)) {
			ClientConvertible convertible = actionLink.getBody();
			ClientDataToJsonConverter factory = dataToJsonConverterFactory
					.factorUsingConvertible(convertible);
			internalLinkBuilder.addKeyJsonObjectBuilder("body", factory.toJsonObjectBuilder());
		}
	}

	private boolean actionLinkHasBody(ClientActionLink actionLink) {
